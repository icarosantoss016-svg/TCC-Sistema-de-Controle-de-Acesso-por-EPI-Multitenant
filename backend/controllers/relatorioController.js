...
const { LogAcesso, Setor, Empresa, UsuarioSetor } = require('../models')

exports.listarLogsFiltrados = async (req, res) => {
  try {
    const { status, id_setor, limite = 50 } = req.query
    ...
    if (usuarioLogado?.perfil === 'ADM_EMPRESA') {
      ...
      filtroSetor.id_empresa = empresasIds
    ...
    const logs = await LogAcesso.findAll({
      ...
      include: [
        {
          model: Setor,
          where: Object.keys(filtroSetor).length > 0 ? filtroSetor : undefined,
          attributes: ['id_setor', 'nome_setor', 'id_empresa'],
          include: [{ model: Empresa, attributes: ['id_empresa', 'nome'] }],
        },
      ],
    ...
    return res.status(500).json({ error: 'Erro interno ao gerar relatório de logs.' })
  }
}
  ...
  try {
    ...
    infracoes.forEach((log) => {
      ...
      if (typeof esquecidos === 'string') {
        try {
          esquecidos = JSON.parse(esquecidos)
        } catch {
          esquecidos = []
        }
      }
    ...
    return res.status(500).json({ error: 'Erro interno ao gerar ranking de EPIs.' })
  }
}
...
exports.listarSetorMaisInfracoes = async (req, res) => {
  try {
    ...
    const ranking = Object.keys(contagem).map((nome) => {
    ...
    return res.status(500).json({ error: 'Erro interno ao gerar lista de setores.' })
  }
}
